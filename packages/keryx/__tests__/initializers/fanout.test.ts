...
afterEach(async () => {
  ...
  api.tasks.unregisterAction("fanout:child");
  api.tasks.unregisterAction("fanout:failing-child");
  api.tasks.unregisterAction("fanout:second-child");
});
...
describe("fanOut multi-action", () => {
  ...
  test("each job resolves its own queue from its action", async () => {
    const result = await api.actions.fanOut([
      { action: "fanout:child", inputs: { itemId: "1" } },
      { action: "fanout:second-child", inputs: { name: "bob" } },
    ]);
  ...
  test("per-job queue override works", async () => {
    await api.actions.fanOut([
      ...
      { action: "fanout:second-child", inputs: { name: "carol" } },
    ]);
...
describe("with workers", () => {
  ...
  test(
    "fanned-out jobs are processed and results collected",
    async () => {
  ...
  test(
    "multi-action fan-out jobs are processed and results collected",
    async () => {
      const result = await api.actions.fanOut([
  ...
  test(
    "failed child jobs have errors collected",
    async () => {
