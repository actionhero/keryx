...
describe("keryx upgrade (CLI integration)", () => {
  ...
  test("does not touch user-owned files", async () => {
    // actions/user.ts is a user-owned file generated by the auth scaffold
    const userActionPath = path.join(projectDir, "actions/user.ts");
    const indexPath = path.join(projectDir, "index.ts");
    const envPath = path.join(projectDir, ".env.example");

    const userActionBefore = fs.readFileSync(userActionPath, "utf-8");
    ...
    expect(fs.readFileSync(userActionPath, "utf-8")).toBe(userActionBefore);
