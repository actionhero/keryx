...
5. `connection.act()` executes the action through the standard middleware pipeline. During `run()`, the action may send a nested `elicitation/create` request if the client advertised that mode. A `McpUrlElicitationRequiredError` is mapped to JSON-RPC `-32042` so the client can finish a URL flow and retry.
