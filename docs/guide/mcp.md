...
- The `redirect_uri` on the authorization request matches one in the document exactly — except for the port on `http:` loopback URIs, which is ignored per [RFC 8252 §7.3](https://datatracker.ietf.org/doc/html/rfc8252#section-7.3). A CLI client can therefore declare `http://localhost/callback` and authorize with `http://localhost:49152/callback`.
